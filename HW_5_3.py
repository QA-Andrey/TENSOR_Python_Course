...
def everything_for_your_cat(cats_data):
    """
    Принимаем список кортежей, где каждый кортеж состоит из: Кличка котика, Возраст котика, Имя покупателя,
    Фамилия покупателя. Имена некоторых владельцев повторяются, потому что у них несколько кошек.
    И возвращаем строку таким образом, чтобы для каждого владельца при выводе данные о всех его животных
    отображались в одной строке, на новой строке следующий покупатель и все его питомцы и т.д.
    Формат для одного покупателя:
    Имя_покупателя Фамилия_покупателя: Кличка_котика1, Возраст; ...; Кличка_котикаN, Возраст\n
    Например (Ввод --> Вывод):
    Ввод
    ...
    Алексей Егоров: Мартин, 5; Вася, 4
    Анна Самохина: Фродо, 3
    :param cats_data: Принимаемый список кортежей.
    :return: Возвращаемая строка в соответветствии с требованиями задачи.
    """
    my_dict = {}
    our_str = ''

    for i, v in enumerate(cats_data):
        if (v[2] + ' ' + v[3]) in my_dict.keys():
            my_dict[v[2] + ' ' + v[3]] = my_dict[v[2] + ' ' + v[3]] + '; ' + v[0] + ', ' + str(v[1])
        else:
