...
def repeats(our_str):
    """
    Принимаем на вход строку, смотрим, сколько раз каждый символ уже встречался,
    и добавляем количество к символам с помощью постфикса формата _n.
    Возвращаем новая строка
    Например (Ввод --> Вывод):
    'letter' --> l_1e_1t_1t_2e_2_r_1
    :param our_str: Принимаемая строка
    :return: Возвращаемя строка в соответствии с требованиями.
    """
    new_str = ''

    for i, v in enumerate(our_str):
        new_str = new_str + v + '_' + str(our_str[:i + 1].count(v))
        print(new_str)
